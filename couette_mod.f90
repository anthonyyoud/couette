program couette_mod
  ...
  double precision, dimension(2*nx1+nx1+1, nx1*nz1) :: p_mat
  type (mat_comp) :: Ux, Zx
  type (uz_mat_comp) :: Uz
  type (zz_mat_comp) :: Zz
  logical :: state_exist
  ...
  else if (abs(tau - 1d0) < epsilon(tau)) then
    write(6, '(A7, f4.2, A22)') 'tau = ', tau, '- Finite aspect ratio'
  else if ((abs(tau - 0d0) > epsilon(tau)) .and. &
           (abs(tau - 1d0) > epsilon(tau))) then
  ...
  else if (abs(tau - 1d0) < epsilon(tau)) then   !or infinite cylinders
  ...
  else if (abs(tau - 1d0) < epsilon(tau)) then     !left-hand side matrices
    call fin_b_mat_setup(b_mat, b_pivot)              !for stream-function,
    call fin_j_mat_setup(j_mat, j_pivot)              !current and magnetic
  else if ((abs(tau - 0d0) > epsilon(tau)) .and. & !Poisson equations
           (abs(tau - 1d0) > epsilon(tau))) then
  ...
  do p = p_start, Ntot        !start main time loop
    ...
    if (savevapor3d) then   !save 3D surface
      if (mod(p, save_rate_2) == 0) then
