module current 
  ...
  subroutine j_poisson(p_mat, jn, j_mat, IPIV)
    ...
    real    (r2),   intent(in)  :: p_mat(0:nx,0:nz), &
                                   j_mat(2*nx1+nx1+1,nx1*nzp1)
    real    (r2),   intent(out) :: jn(0:nx,0:nz)
    real    (r2)                :: p_vec(nx1*nzp1)
    integer (i1),   intent(in)  :: IPIV(nx1*nzp1)
    integer (i1)                :: j, k, info
    type    (deriv)             :: dp
    ...
    do k = 0, nz
      do j = 1, nx1
        p_vec(nx1*k+j) = dx2 * dz2 * &
                        (0.5_r2 * dp%zzz(j,k) / (s(j) * delz**3) + &
                        0.5_r2 * dp%zxx(j,k) / (s(j) * dx2 * delz) - &
                        0.25_r2 * one_eta * dp%zx(j,k) / &
    ...
    select case (r2)
      ...
      case (DPr)
        call DGBTRS('N', nx1*nzp1, nx1, nx1, 1, j_mat, 2*nx1+nx1+1, &
                     IPIV, p_vec, nx1*nzp1, info)
        if (info /= 0) then
          print*, 'ERROR: DPr solve error cur_inf_DGBTRS, INFO=', info
          stop
        end if
      case default
        stop 'ERROR: Precision selection error - current.f90 infinite *GBTRS'
    end select
  ...
  subroutine fin_j_poisson(p_mat, jn, j_mat, IPIV)
    ...
    do k = 1, nz1
      do j = 1, nx1
        p_vec(nx1*(k-1)+j) = dx2 * dz2 * (0.5_r2 * dp%zzz(j,k) / &
                             (s(j) * delz**3) + &
                             0.5_r2 * dp%zxx(j,k) / (s(j) * dx2 * delz) - &
                             0.25_r2 * one_eta * dp%zx(j,k) / &
                             (s(j)**2 * delx * delz))
    ...
    select case (r2)
      ...
      case (DPr)
        call DGBTRS('N', nx1*nz1, nx1, nx1, 1, j_mat, 2*nx1+nx1+1, &
                     IPIV, p_vec, nx1*nz1, info)
        if (info /= 0) then
          print*, 'ERROR: DPr solve error cur_fin_DGBTRS, INFO=', info
          stop
        end if
      case default
        stop 'ERROR: Precision selection error - current.f90 finite *GBTRS'
    end select
