module solve
  ...
  subroutine solve_ux(uo, u, u_nl, t, ux)
    ...
    real, intent(in) :: t, u(0:nx,0:nz), u_nl(0:nx,0:nz)
    real, intent(inout) :: uo(0:nx,0:nz)
    ...
    do k = 1, nz1
      ...
      ux_rhs(1) = ux_rhs(1) + 0.5 * rxx * uo(0,k) - &
                  one_eta * rx * 0.25 * uo(0,k) / s(1)    !BCS
      ux_rhs(nx1) = ux_rhs(nx1) + 0.5 * rxx * uo(nx,k) + &
                    one_eta * rx * 0.25 * uo(nx,k) / s(nx1)
    ...
    if (abs(tau - 1.0) > epsilon(tau)) then !extra entries due to BCS
      do k = 0, nz, nz
        ux_rhs(:) = u(1:nx1,k) + u_nl(1:nx1,k)

        ux_rhs(1) = ux_rhs(1) + 0.5 * rxx * uo(0,k) - &
                    one_eta * rx * 0.25 * uo(0,k) / s(1)
        ux_rhs(nx1) = ux_rhs(nx1) + 0.5 * rxx * uo(nx,k) + &
                      one_eta * rx * 0.25 * uo(nx,k) / s(nx1)
  ...
  subroutine solve_Zx(zo, zn, z_nl, po, t, zx)
    ...
    real, intent(in) :: t, zn(0:nx,0:nz), po(0:nx,0:nz), &
                        z_nl(0:nx,0:nz)
    real, intent(inout) :: zo(0:nx,0:nz)
    ...
    do k = 1, nz1
      ...
      zx_rhs(1) = zx_rhs(1) + 0.5 * rxx * zo(0,k) - &
                  one_eta * rx * 0.25 * zo(0,k) / s(1)   !BCS
      zx_rhs(nx1) = zx_rhs(nx1) + 0.5 * rxx * zo(nx,k) + &
                    one_eta * rx * 0.25 * zo(nx,k) / s(nx1)
  ...
  subroutine solve_uz(uo, u, t, uz)
    ...
    real, intent(in) :: t, uo(0:nx,0:nz)
    real, intent(inout) :: u(0:nx,0:nz)
    type (uz_mat_comp), intent(in) :: uz
    integer :: j, k
    real :: uz_rhs(0:nz), uz_rhs_t1(nz1), &
    ...
    if (abs(tau - 1.0) < epsilon(tau)) then
      ...
      do j = 1, nx1
        ...
        uz_rhs_t1(1) = uz_rhs_t1(1) + 0.5 * rzz * u(j,0)
        uz_rhs_t1(nz1) = uz_rhs_t1(nz1) + 0.5 * rzz * u(j,nz)   !BCS
  ...
  subroutine solve_Zz(zo, po, zn, t, zz)
    ...
    real, intent(in) :: t, zo(0:nx,0:nz), po(0:nx,0:nz)
    real, intent(inout) :: zn(0:nx,0:nz)
    ...
    do j = 1, nx1
      ...
      Zz_rhs(1) = Zz_rhs(1) + 0.5 * rzz * zn(j,0)
      Zz_rhs(nz1) = Zz_rhs(nz1) + 0.5 * rzz * zn(j,nz)   !BCS
  ...
  subroutine thomas (lb, m, up, di, lo, r)
    ...
    real, intent(in) :: up(lb:m-1), di(lb:m), lo(lb+1:m)
    real, intent(inout) :: r(lb:m)
    real :: dnew(lb:m), aa = 0.0
    integer :: j
