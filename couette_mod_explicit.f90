...
call get_new_u(unew, uold, pold, A, s, t)
...
SUBROUTINE get_new_u(u, uo, po, A, s, t)
use parameters
use derivs
implicit none
...
do j = 1, nx-1
   u(j,:) = rxx * uo_xx(j,:) + &
            (0.5d0 * rx * (1d0 - eta) / s(j)) * uo_x(j,:) - &
            ((1d0 - eta)**2 * dt / s(j)**2) * uo(j,:) + &
            rzz * uo_zz(j,:) + uo(j,:) - &
            ((1d0 * 0.25d0 * rx) / (s(j) * delz)) * &
            (po_x(j,:) * uo_z(j,:) - po_z(j,:) * uo_x(j,:)) + &
...
do j = 1, nx-1
   zn(j,:) = rxx * zo_xx(j,:) + &
        ...
        rzz * zo_zz(j,:) + zo(j,:) + &
        ((1d0 - eta) * rz / s(j)**2) * (vc(j) + 1d0 * uo(j,:)) * &
        uo_z(j,:) - &
        ((1d0 * 0.25d0 * rx) / (s(j) * delz)) * &
